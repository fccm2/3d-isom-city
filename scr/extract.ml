...
let imgs =
  let d = Sys.readdir "cropped" in
  let sep = Filename.dir_sep in
  Array.fold_left (fun acc fn ->
    if Filename.check_suffix fn ".png"
    then begin
      let name = Filename.chop_suffix fn ".png" in
      let fn1 = Printf.sprintf "assets%s%s.png" sep name in
      let fn2 = Printf.sprintf "cropped%s%s.png" sep name in
      let fn3 = Printf.sprintf "cropped%s%s.crops" sep name in
      let fn4 = Printf.sprintf "intermediate%s%s" sep name in
